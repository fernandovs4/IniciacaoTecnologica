...
const ChartClinicasCondicao = () => {
  const [dados, setDados] = useState({});
  const [selectedHospital, setselectedHospital] = useState("A.C. Camargo");
  const [chartData, setChartData] = useState({});

  useEffect(() => {
    fetch('http://localhost:5000/construirTabela?&stdage=todas&fase=todas&gender=todas&tipo=clinica_condicao&inversed=true&simetric=false&sort_externo=true&sort_interno=true', {
      method: 'GET',
      headers: {
...
export default ChartClinicasCondicao;
